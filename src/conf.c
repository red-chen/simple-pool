...
static void conf_node_destroy(Conf* conf)
{
    if (conf->head != NULL)
    {
        struct conf_node_t* cur = conf->head;
        struct conf_node_t* tmp;
...
{
    for (struct conf_node_t* cursor = conf->head; cursor != NULL; cursor = cursor->next)
    {
...
void conf_set(Conf* conf, const char* key, const char* value)
{
    ...
    struct conf_node_t* node = malloc(sizeof(struct conf_node_t));
    bzero(node, sizeof(struct conf_node_t));
